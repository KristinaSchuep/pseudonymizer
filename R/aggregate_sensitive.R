...
aggregate_sensitive <- function(data,
  ...
  index <- ifelse(nchar(data[, var_plz]) %in% 6 & grepl("^8", data[, var_plz]) ,TRUE,FALSE)

  # force overwrite NAs in index
  index <- index %in% TRUE

  data[index,var_new_plz] <- gsub('.{2}$', '', data[index, var_plz])

  # for all others just keep original zip code
  data[!index, var_new_plz] <- data[!index, var_plz]
