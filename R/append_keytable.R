...
#' @export
append_keytable <- function(df, path, sensitive){
  ...
  newest<-sort(list.files(file.path(path, "keytable"), pattern = "keytable.csv"),decreasing = TRUE)[1]
    ...
    write.csv(keytable_temp,file.path(path,"keytable",name), row.names = FALSE)
    message("No existing keytable, current keytable is saved")
      ...
      keytable <- read.csv(file.path(path, "keytable", newest),colClasses = c('character','character','character','character', 'POSIXct', 'POSIXct'))
